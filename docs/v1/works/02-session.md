...
**id 是身份,不是标签。** 调用方从自己的世界算出这个 id(一个块、一个 job、
一个仓库路径),靠它重入([§2.1](#21-为什么-id-由调用方给))。

改名意味着身份变了 —— 而调用方下次还是会按同样的规则算出**原来那个 id**,
然后找不到现场。**rename 直接破坏 id 存在的唯一理由:同一个 id 永远指向同一个现场。**

tmux 里 `rename-session` 有意义,因为 name 是给人看的标签;
这里 id 是调用方算出来的键,**键不改名**。

真要换 id:`kill()` 掉再建一个。显式的,而且调用方会被迫想清楚
"那旧 id 指向的东西还要不要"。
