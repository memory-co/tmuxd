...
- **`-t` / `--session` / `--target` 在 2.0 直接删掉**,不留隐藏别名。
  一个 `--help` 和文档里都查不到的别名,是个没人找得到的承诺;
  2.0 本来就是有意破坏兼容的一版([CHANGELOG](../../../CHANGELOG.md))。
...
`-t` / `--session` / `--target` **删了,没有隐藏别名**。

留一个查不到的别名,和删掉它,对使用者其实是同一件事:两种情况下他都得看
[CHANGELOG](../../../CHANGELOG.md) 才知道发生了什么。既然如此,
不如让错误立刻发生 —— `-t` 现在是明确的用法错误,而不是一个悄悄还能用的旧写法。

**这一版整体是有意破坏兼容的**,别的改动见 CHANGELOG。
