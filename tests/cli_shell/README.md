...
1. **参数真的递下去了**:`-s` / `-c` / `-e` / `--` 后面的命令。
   尤其是 `--` —— `argparse.REMAINDER` 会把分隔符本身一起交回来,
   不剥掉的话 `new -s x -- cat` 实际在跑命令 `-- cat`。**这条曾经真的写错过。**
2. **退出码是接口**,不是装饰:脚本靠它判断,不该去解析输出。
   `has` 用 3 表示"不在"——那是答案不是错误。
3. **错误走 stderr**,stdout 上只有正常输出,这样 `tmuxd url -t x | xargs open` 才安全。
4. **`serve` / `start` / `stop` 是同一份逻辑套不同的壳**,而 `stop` 停的是门面 ——
   会话还在。这条在 [`survives_the_process/`](../survives_the_process/) 从库那边验过,
   这里从命令行这边再验一次,因为**用户读到的是 CLI 打印的那句话**。
5. **配置文件只是构造参数的另一种写法**,不是第二套东西。
6. **`-H` 远端模式能做的事少一档**:`serve` / `start` / `stop` 在那下面直接拒绝,
   因为进程生命周期是对面那台机器的事。
